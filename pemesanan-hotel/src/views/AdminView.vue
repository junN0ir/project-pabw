...
async function submitMitra() {
  ...
  let result
  if (editMode.value) {
    // Edit: delegasikan ke adminStore (validasi email duplikat + hotel conflict dilakukan di store)
    result = admin.editMitra(editingId.value, { ...mitraForm })
  } else {
    // Tambah: delegasikan ke adminStore
    result = admin.tambahMitra({ ...mitraForm })
  }

  if (!result.success) {
    // Data tidak valid → tampilkan pesan error dari store
...
async function eksekusiHapus() {
  ...
  // Simulasi delay cek reservasi
  await new Promise(r => setTimeout(r, 1000))

  // Delegasikan ke store (cek reservasi aktif ada di store)
  const result = admin.hapusMitra(mitraToDelete.value?.id)
  hapusLoading.value = false

  if (!result.success) {
    // Alt: hapus gagal → tampilkan pesan tidak bisa dihapus
...
async function loadReservasi() {
  if (!selectedHotelFilter.value) return
