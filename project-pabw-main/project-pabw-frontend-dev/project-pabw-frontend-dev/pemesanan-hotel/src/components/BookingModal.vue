...
function handleSubmit() {
  ...
  const guestCount = form.rooms * store.currentRoom.capacity

  const saveLocalBooking = (reservationData = {}) => {
    store.saveBooking({
      id_history: reservationData.id_history,
      room: reservationData.type_room || store.currentRoom.name,
      hotelId: reservationData.id_list_hotel || store.currentRoom.hotelId,
      hotelName: reservationData.hotel_name || store.currentRoom.hotelName,
      ...
      total: reservationData.amount || summary.total,
      bookingDate: new Date().toISOString(),
      status: reservationData.status || 'confirmed'
    })
  }

  if (auth.user?.id && store.currentRoom.id_list_kamar) {
    apiPost('/reservations/book', {
      id_user: auth.user.id,
      id_list_kamar: store.currentRoom.id_list_kamar,
