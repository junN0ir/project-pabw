    ...
    <div class="auth-card">
      ...
      <form v-if="mode === 'login'" @submit.prevent="handleLogin" class="auth-form">
        ...
        <div class="form-group">
          ...
          <input type="text" v-model="loginForm.email" placeholder="email@contoh.com" required />
        </div>
        <div class="form-group">
        ...
        <p class="demo-hint">💡 Gunakan akun yang sudah ada di database backend Anda.</p>
      </form>
      ...
      <div v-else class="auth-form">
        ...
        <div class="form-group">
          ...
          <input type="password" v-model="forgotPassword" placeholder="Min. 6 karakter" minlength="6" />
        </div>
        <p v-if="forgotError" class="error-msg">{{ forgotError }}</p>
        <button class="btn btn-primary btn-full" @click="handleForgot">Reset Password</button>
...
async function handleRegister() {
  ...
  loading.value = true
  const result = await auth.register(regForm.value.name, regForm.value.email, regForm.value.password, regForm.value.phone)
  loading.value = false
  if (result.success) {
    router.push('/')
  } else {
